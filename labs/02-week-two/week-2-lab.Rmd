...
class(loans_full_schema$homeownership) # factor
class(loans_full_schema$application_type) # factor
class(loans_full_schema$interest_rate) # numeric
max(loans_full_schema$interest_rate) # 30.9
min(loans_full_schema$interest_rate) # 5.31
mean(loans_full_schema$interest_rate) # 12.4 this is pretty high...they're all pretty high
```
...
homeownership <- ggplot(data = loans, aes(x = homeownership)) +
  geom_bar() +
  labs(x = "Homeownership", y = "Count")

homeownership

homeownership_proportion <- loans %>% 
  count(homeownership) %>% 
  mutate(proportion = n / sum(n))

homeownership_proportion_plot <- ggplot(data = homeownership_proportion, aes(x = homeownership, y = proportion)) +
  geom_col() +
  labs(x = "Homeownership", y = "Proportion")

homeownership + homeownership_proportion_plot
```
...
homeownership_application_stacked <- ggplot(data = loans, aes(x = homeownership, fill = application_type)) +
  geom_bar() +
  scale_fill_manual(values = c(IMSCOL["blue", "full"], IMSCOL["yellow", "full"])) +
  labs(x = "Homeownership", y = "Count", fill = "Application type") 

homeownership_application_stacked
```
