...
rootdir <- here::here("labs", "_common.R")
...
```{r}
# (i) total length as a function of tail length
possum_tail_tot <- ggplot(data = possum, aes(x = tail_l, y = total_l)) +
  geom_point() +
...
# (ii) head length as a function of tail length
possum_tail_head <- ggplot(data = possum, aes(x = tail_l, y = head_l)) +
  geom_point() +
  labs(x = "Tail Length (cm)",
       y = "Head Length (cm)") +
  theme_light()
...
The relationship between tail length and total length, and tail length and head length appears to have a positive correlation. The strength of the correlation appears strong between both tail and total, and tail and head, but stronger with tail and total.
...
```{r}
# total_l is y, tail_l is x, and possum is data
# the formula we're using is y = beta x + m
lm(total_l ~ tail_l, possum) 
```

2. Interpret your two coefficients, paying careful attention to units. 

41.04 is our $\beta_0$ and is the constant and 1.24 is the $\hat\beta_1$, which is the slope. This means that as the slope increases by one unit (in this case cm) the possum total length increases by 1.24. 

3. Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$? 

The challenge with interpreting $\beta_0$ is that when you look at a possum with a zero tail length  you would still have a total length. And it doesn't make sense to have a possum with no tail. So we need to make sure we don't interpret things outside our model. 
...
Because the slope is 1.24, this means that as we increase the tail length by one unit we are also increasing a portion of the body length by some unit (in this case think of the unit increase as 0.24 cm). And this is because the slope is typically a one-to-one relationship. 
...
predictions <- possum %>% add_predictions(model) %>%
